...
(() => {
  ...
  function createRuntime(doc = document) {
    ...
    function ensureFetched(name) {
      ...
      if (r.fetched) return;
      r.fetched = true;
      const urlHtml = COMPONENT_DIR + "/" + encodeURIComponent(name) + ".html";
      const urlDc = COMPONENT_DIR + "/" + encodeURIComponent(name) + ".dc.html";
      // Prefer the .html sibling (static export), fall back to the .dc.html source.
      fetch(urlHtml).then((res) => {
        if (res.ok) return res.text();
        // if .html not found, try .dc.html
        if (res.status === 404) return fetch(urlDc).then((res2) => {
          if (!res2.ok) {
            console.error(
          ...
          console.error(
            "[dc-runtime] sibling fetch for <" + name + "/> threw when trying",
            urlDc,
            e2
          );
          return "";
        ...
        console.error(
          "[dc-runtime] sibling fetch for <" + name + "/> failed when fetching:",
          urlHtml,
          "returned",
          res.status,
          "\u2014 skipping."
        ...
        if (!parsed) {
          console.error(
            ...
            (t && t.indexOf('<x-dc') === -1) ? (urlHtml + ' or ' + urlDc) : (urlHtml),
